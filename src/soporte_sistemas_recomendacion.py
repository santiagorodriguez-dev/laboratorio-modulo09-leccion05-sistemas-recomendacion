...
def filter_data(df):
    ...
    """
    ## Ratings Per Movie
    ratings_per_movie = df.groupby('game_name')['rank'].count()
    ## Ratings By Each User

    ratings_per_movie_df = pd.DataFrame(ratings_per_movie)
   
    filtered_ratings_per_movie_df = ratings_per_movie_df
    
    df = df[df.game_name.isin(filtered_ratings_per_movie_df)]
